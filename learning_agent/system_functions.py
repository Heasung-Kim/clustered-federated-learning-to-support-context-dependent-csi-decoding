...
def result_analysis_vqvae_compression(model, data_loader, device, single_batch=False):
    ...
    with torch.no_grad():
        for i, (x, y, x_sf) in enumerate(data_loader):
            x, y, H_SF_sample = x.to(device), y.to(device), x_sf.to(device)

            embedding_loss, x_hat, codeword = model(x)
    ...
    for sample_idx in range(1):
        ...
        # Plot the second tensor as a heatmap
        axs[2].imshow(abs(recovered_imgs[sample_idx]), cmap='viridis')
        ...
        # Display the plots debug
        plt.savefig('sample{}.pdf'.format(sample_idx), bbox_inches='tight')
        plt.show()
    """
...
def decoder_model_averaging(model_weight_address, client_cluster):
