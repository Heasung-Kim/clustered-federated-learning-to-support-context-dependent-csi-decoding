...
from global_config import global_logger, logging, ROOT_DIRECTORY
...
class CentralUnit:
    def __init__(self,
        ...
        self.load_initial_model = load_initial_model
        self.initial_model_path = initial_model_path
        self.load = load
        self.encoder_load = True
        ...
        if subprotocol == "decoder_only_averaging" and self.encoder_load is True:
            self.encoder_model_path = self.initial_model_path #os.path.join(ROOT_DIRECTORY, "results", model_name, "model.pt")
            for m_idx, model in enumerate(self.models):
                state_dict = torch.load(self.encoder_model_path)
                filtered_state_dict = {k: v for k, v in state_dict.items() if 'encoder' in k}
            ...
            for c_idx, client in enumerate(self.clients):
                state_dict = torch.load(self.encoder_model_path)
                filtered_state_dict = {k: v for k, v in state_dict.items() if 'encoder' in k}
                client.local_model.load_state_dict(filtered_state_dict, strict=False)
                for name, param in client.local_model.named_parameters():
                    if "encoder" in name:
                        param.requires_grad = False
            global_logger.info("Load encoder model success")
        else:
            global_logger.info("Fail to load encoder model")

        if self.load is True:
            with open(os.path.join(self.data_tracking_path, "metrics.pickle"), 'rb') as handle:
    ...
    def run_CFLGP(self):
        ...
        for c_round in range(self.n_communication_rounds):
            ...
            for client_idx, client in enumerate(tqdm(self.clients)):
                ...
                for model_idx in query_model_indices:
                    ...
                    if c_round % self.clustering_period == 0 and model_idx == criterion_model_index and CLUSTERING_CONVERGENCE is not True:
                        ...
                        gradient_profile_matrix[(criterion_model_index) * self.n_params_compressed_gradient: (criterion_model_index + 1) * self.n_params_compressed_gradient,
                        client_idx] = gradient_profile_matrix[
                                      (criterion_model_index) * self.n_params_compressed_gradient: (criterion_model_index + 1) * self.n_params_compressed_gradient,
                                      client_idx] * (1 - beta) \
            ...
            # Clustering & Matching
            # ====================================
            if c_round % self.clustering_period == 0 and c_round < self.clustering_termination_threshold:
                # global_logger.info("Spectral Clustering.")
                info = spectral_clustering_and_matching(gradient_profile_matrix, n_centers=None if self.unknown_k else self.n_models,
                ...
                criterion_model_index = (criterion_model_index + 1) % np.max(self.estimated_cluster_ids_new)
                self.estimated_cluster_ids = self.estimated_cluster_ids_new if c_round > self.warmup_epoch else np.zeros(shape=self.n_clients, dtype=int)
    ...
    def run_LOCAL(self):
        self.estimated_cluster_ids = np.arange(self.n_clients)
        for c_round in range(self.n_communication_rounds):
            if c_round % self.evaluation_interval == 0 or c_round == self.n_communication_rounds - 1:
                client_acc = np.zeros(shape=self.n_clients)
                for client_idx, client in enumerate(tqdm(self.clients)):
            ...
            for client_idx, client in enumerate(self.clients):
                estimated_cluster_id = self.estimated_cluster_ids[client_idx]
                if self.protocol == "model_averaging":
                    _ = client.local_update(device=self.device)
                else:
                    _ = client.local_update(device=self.device, single_batch=True)
            global_logger.info("cluster ids: {}".format(self.estimated_cluster_ids))
