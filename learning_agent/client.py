...
class Client:
    def __init__(self, model_class, optimizer, local_dataset, test_dataset, use_scheduler, batch_size,
        ...
        n_total_params = sum(p.numel() for p in self.local_model.parameters())
        ...
        if self.use_scheduler is True:
            #self.scheduler = CosineAnnealingWarmRestarts(self.local_optimizer, 20, 2, eta_min=1e-5)
            self.scheduler = MultiStepLR(self.local_optimizer, milestones=[10000//batch_size*100], gamma=0.1)
    ...
    def get_train_dataset_variance(self):
        variance = 0.
        cnt = 0
        for i, (x, y,_ ) in enumerate(self.train_dataset_loader):
            cnt += 1
            variance += x.var()
        variance /= cnt
        return variance
